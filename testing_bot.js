...
const data_bot = new Telegraf(process.env.TELEGRAM_TESTING_BOT_TOKEN);
...
data_bot.hears(/^jan|feb|mar|apr|may|jun|jul|aug|sep|oct|nov|dec$/, async ctx => {
    ...
    let detailed_bill_string = ''
    for (const [category, items] of Object.entries(category_details_copy)) {
        detailed_bill_string.concat(`<b>Auto</b>: $${category_details_copy[category] ? category_details_copy[category] : 0}, ${category_details[category] ? category_details[category].length:0}, ${category_details_copy[category] ? (100*(category_details_copy[category]/sum_by_month)).toFixed(2) : 0}%`)
    }

    data_bot.telegram.sendMessage(ctx.chat.id, `You have spent ${sum_by_month}/${month_target} in ${month}`)
    data_bot.telegram.sendMessage(ctx.chat.id, detailed_bill_string)
